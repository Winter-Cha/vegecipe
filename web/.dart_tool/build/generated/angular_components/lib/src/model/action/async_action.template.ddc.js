define(['dart_sdk'], function(dart_sdk) {
  'use strict';
  const core = dart_sdk.core;
  const dart = dart_sdk.dart;
  const dartx = dart_sdk.dartx;
  const src__model__action__async_action$46template = Object.create(dart.library);
  const src__model__action__async_action_controller$46template = Object.create(dart.library);
  const src__model__action__delegating_async_action$46template = Object.create(dart.library);
  src__model__action__async_action$46template.initReflector = function() {
  };
  dart.defineLazy(src__model__action__async_action_controller$46template, {
    /*src__model__action__async_action_controller$46template._visited*/get _visited() {
      return false;
    },
    set _visited(_) {}
  });
  src__model__action__async_action_controller$46template.initReflector = function() {
    if (dart.test(src__model__action__async_action_controller$46template._visited)) {
      return;
    }
    src__model__action__async_action_controller$46template._visited = true;
    src__model__action__async_action$46template.initReflector();
  };
  dart.defineLazy(src__model__action__delegating_async_action$46template, {
    /*src__model__action__delegating_async_action$46template._visited*/get _visited() {
      return false;
    },
    set _visited(_) {}
  });
  src__model__action__delegating_async_action$46template.initReflector = function() {
    if (dart.test(src__model__action__delegating_async_action$46template._visited)) {
      return;
    }
    src__model__action__delegating_async_action$46template._visited = true;
    src__model__action__async_action$46template.initReflector();
  };
  dart.trackLibraries("packages/angular_components/src/model/action/async_action.template.ddc", {
    "package:angular_components/src/model/action/async_action.template.dart": src__model__action__async_action$46template,
    "package:angular_components/src/model/action/async_action_controller.template.dart": src__model__action__async_action_controller$46template,
    "package:angular_components/src/model/action/delegating_async_action.template.dart": src__model__action__delegating_async_action$46template
  }, '{"version":3,"sourceRoot":"","sources":["async_action.template.dart","async_action_controller.template.dart","delegating_async_action.template.dart"],"names":[],"mappings":";;;;;;;;;EAQsB;;MCAlB,+DAAQ;YAAG;;;;;AAEb,kBAAI,+DAAQ,GAAE;AACZ;;AAEF,sEAAW;AAEX,IAAM,yDAAa;EACrB;;MCRI,+DAAQ;YAAG;;;;;AAEb,kBAAI,+DAAQ,GAAE;AACZ;;AAEF,sEAAW;AAEX,IAAM,yDAAa;EACrB","file":"async_action.template.ddc.js"}');
  // Exports:
  return {
    src__model__action__async_action$46template: src__model__action__async_action$46template,
    src__model__action__async_action_controller$46template: src__model__action__async_action_controller$46template,
    src__model__action__delegating_async_action$46template: src__model__action__delegating_async_action$46template
  };
});

//# sourceMappingURL=async_action.template.ddc.js.map
