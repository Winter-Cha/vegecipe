define(['dart_sdk', 'packages/quiver/check', 'packages/intl/intl', 'packages/angular/src/core/change_detection/change_detection', 'packages/angular_components/interfaces/has_disabled', 'packages/angular_components/utils/browser/dom_service/dom_service'], function(dart_sdk, check, intl, change_detection, has_disabled, dom_service) {
  ...
  const material_slider__material_slider = Object.create(dart.library);
  const $modulo = dartx['%'];
  ...
  const _changeDetector = dart.privateName(material_slider__material_slider, "_changeDetector");
  const _domService = dart.privateName(material_slider__material_slider, "_domService");
  const _changeController = dart.privateName(material_slider__material_slider, "_changeController");
  const _divisible = dart.privateName(material_slider__material_slider, "_divisible");
  const _setValueToMousePosition = dart.privateName(material_slider__material_slider, "_setValueToMousePosition");
  material_slider__material_slider.MaterialSliderComponent = class MaterialSliderComponent extends core.Object {
    ...
    ngAfterChanges() {
      if (!dart.test(dart.fn(() => {
        ...
        return true;
      }, VoidTobool())())) dart.assertFailed(null, "file:///private/var/folders/_q/d3_d45gx0clfh53k0fj1d8sw0000gn/T/scratch_spaceN42JPh/packages/angular_components/material_slider/material_slider.dart", 87, 12, "() {checkArgument(min < max, message: 'Failed assertion: ${min} < ${max}'); checkArgument(step > 0, message: 'Failed assertion: ${step} > 0'); checkArgument(_divisible(max - min, step), message: 'Failed assertion: (${max} - ${min}) % ${step} ~ 0.'); checkArgument(value >= min, message: 'Failed assertion: ${value} >= ${min}'); checkArgument(value <= max, message: 'Failed assertion: ${value} <= ${max}'); checkArgument(_divisible(value - min, step), message: 'Failed assertion: (${value} - ${min}) % ${step} ~ 0.'); return true;}()");
    }
  ...
  dart.setLibraryUri(material_slider__material_slider.MaterialSliderComponent, "package:angular_components/material_slider/material_slider.dart");
