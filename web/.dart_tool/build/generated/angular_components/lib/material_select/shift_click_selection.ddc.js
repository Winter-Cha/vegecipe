define(['dart_sdk', 'packages/angular_components/model/selection/selection_model', 'packages/angular_components/material_select/activation_handler'], function(dart_sdk, selection_model, activation_handler) {
  ...
  const material_select__shift_click_selection = Object.create(dart.library);
  const $indexOf = dartx.indexOf;
  const $forEach = dartx.forEach;
  const $abs = dartx.abs;
  const $take = dartx.take;
  const $skip = dartx.skip;
  const _pivot = dart.privateName(material_select__shift_click_selection, "_pivot");
  const _handleClick = dart.privateName(material_select__shift_click_selection, "_handleClick");
