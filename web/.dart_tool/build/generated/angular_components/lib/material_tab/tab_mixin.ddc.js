define(['dart_sdk'], function(dart_sdk) {
  ...
  const material_tab__tab_mixin = Object.create(dart.library);
  const $offsetWidth = dartx.offsetWidth;
  const $offsetLeft = dartx.offsetLeft;
  const $width = dartx.width;
  const $isNotEmpty = dartx.isNotEmpty;
  const _label = dart.privateName(material_tab__tab_mixin, "_label");
  const _textWidth = dart.privateName(material_tab__tab_mixin, "_textWidth");
